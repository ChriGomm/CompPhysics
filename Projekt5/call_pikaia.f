C Hinweis zum CP-Kurs: dieses Programm ist für den Aufruf mit
C Fortran alleine gedacht. Es enthält das Hauptprogramm xpikaia mit zwei
C Beispielfunktionen prob2 und twod.
C Diese Routinen müssen entfernt/auskommentiert werden, wenn das
C aufrufende Programm in C++ geschrieben ist, sonst gibt es beim Linken
C Probleme (zweimal "main")

c       program xpkaia
c c======================================================================
c c     Sample driver program for pikaia.f
c c======================================================================
c       implicit none
c       integer n, seed, i, status
c       parameter (n=2)
c       real ctrl(12), x(n), f, twod
c       external twod, prob2
c c
c c        (twod is an example fitness function, a smooth 2-d landscape)
c c
c c     First, initialize the random-number generator
c c
...
c c     Now call pikaia
c       call pikaia(prob2,n,ctrl,x,f,status)
c c     call pikaia(twod,n,ctrl,x,f,status)
c c
c c
...
c       write(*,*) '      f: ',f
c       write(*,20) ctrl
c  20   format(   '    ctrl: ',6f9.5/10x,6f9.5)
c c
c       goto 1
c       end
c c*********************************************************************
c       function prob2(n,x)
c c=====================================================================
c c     Compute sample fitness function (2 Gauss functions)
c c=====================================================================
c       implicit none
c c
c c     Input:
...
c       real prob2
c c
c       if (x(1).gt.1..or.x(2).gt.1.) stop
c       r1=(0.5-x(1))**2+ (0.5-x(2))**2
c       r2=(0.6-x(1))**2+ (0.1-x(2))**2
c       prob2 = 0.8/exp(r1/0.09) + 0.879008/exp(r2/0.03**2)
...
c       function twod(n,x)
c c=====================================================================
c c     Compute sample fitness function (2-d landscape)
c c=====================================================================
c       implicit none
c c
c c     Input:
...
c       real pi,sigma2
c       integer nn
c       parameter (pi=3.1415926536,sigma2=0.15,nn=9)
c c
c c     Local
c       real rr

c       if (x(1).gt.1..or.x(2).gt.1.) stop
c       rr=sqrt( (0.5-x(1))**2+ (0.5-x(2))**2)
c       twod=cos(rr*nn*pi)**2 *exp(-rr**2/sigma2)

c       return
c       end
c*********************************************************************
c ------- bis hierhin löschen, wenn aus C++  aufgerufen wird ---------
